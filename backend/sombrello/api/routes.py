...
class RouteData(BaseModel):
    ...
    Attributes:
        ...
        metadata: Track-level metadata (name/description/type) preserved from the original GPX.
    """
...
async def create_route(request : Request) -> dict:
    ...
    **Errors:**
    Returns HTTP `422` if the GPX content cannot be validated, is malformed,
    or contains a trackpoint with no timestamp.
    """
    gpx_bytes = await request.body()
    gpx_string = gpx_bytes.decode("utf-8")
    try:
        list_trackpoints, metadata = gpx_to_trackpoint_list(gpx_string=gpx_string)
    except ValidationError as e:
        raise HTTPException(status_code=422, detail=e.errors())
    except GPXXMLSyntaxException as e:
        raise HTTPException(status_code=422, detail=f"XML is malformed: {e}")
    except MissingTimestampError as e:
        raise HTTPException(status_code=422, detail=str(e))
        
    route_id = str(uuid4())
    _routes[route_id] = RouteData(trackpoints=list_trackpoints, gpx_string=gpx_string, metadata=metadata)
...
def get_enriched_route(id : str) -> Response:
    ...
    **Errors:**
    ...
    route = _routes[id]
    trackpoints_enriched = _enrich_all(route.trackpoints)
    enriched_gpx = trackpoints_to_gpx(
        trackpoints=trackpoints_enriched,
        gpx_string=route.gpx_string,
